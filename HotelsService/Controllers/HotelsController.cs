...
namespace HotelsService.Controllers
{
    public class HotelsController : ApiController
    {
        ...
        public List<Hotel> Search(string SearchFieldName, string SerachFieldValue)
        {
            ...
            return SearchForHotel(SearchFieldName, SerachFieldValue);
        }

        private List<Hotel> SearchForHotel(string SearchFieldName, string SerachFieldValue)
        {
            ...
            switch (SearchFieldName)
            {
                case "HotelName":
                    {
                        SearchResult = AllHotels.Where(hotel => hotel.HotelName == SerachFieldValue).ToList();
                        break;
                    }
                case "HotelRating":
                    {
                        SearchResult = AllHotels.Where(hotel => hotel.Rating == SerachFieldValue).ToList();
                        break;
                    }
                case "IsReady":
                    {
                        SearchResult = AllHotels.Where(hotel => hotel.IsAvailable == (SerachFieldValue == "Yes" ? true : false)).ToList();
                        break;
                    }
            }
            return SearchResult;
